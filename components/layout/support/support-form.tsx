...
import CreateSupportMailAction from '@/lib/actions/CreateSupportMailAction';
...
export default function SupportForm() {
  ...
  const { mutate: submitFeedback, isPending } = useMutation({
    mutationFn: CreateSupportMailAction,
    onSuccess: ({ success }) => {
      setEmail(session.data?.user.email ?? '');
      setMessage('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    submitFeedback({ email, name, message, subject });
  };
        ...
        <div className='mx-auto max-w-2xl'>
          ...
          <Card className='shadow-sm border-gray-200'>
            <CardHeader>
              ...
              <CardDescription>Fill in your query</CardDescription>
            </CardHeader>
            <CardContent>
